...
class Config(BaseSettings):
    ...
    manager_agent_temperature: float = Field(
        default=0.7,
        description="Temperature for manager agent"
    )
    manager_agent_max_tokens: int = Field(
        default=2048,
        description="Max tokens for manager agent"
    )
    web_surfer_temperature: float = Field(
        default=0.3,
        description="Temperature for web surfer agent"
    )
    web_surfer_max_tokens: int = Field(
        default=1024,
        description="Max tokens for web surfer agent"
    )
    ...
    memory_pruning_threshold: int = Field(
        default=100,
        description="Memory pruning threshold"
    )
